...
function createWaterfall(id, diameter, height, shape, speed, topDark, bottomDark, topLight, bottomLight, foamColor, addFoam, opacity)
{
	if (shadersLoaded && RPM.Scene.Map.current === lastMap)
	{
		...
		setTimeout(createWaterfall, 32, id, diameter, height, shape, speed, topDark, bottomDark, topLight, bottomLight, foamColor, addFoam, opacity);
}

RPM.Manager.Plugins.registerCommand(pluginName, "Create waterfall", (id, diameter, height, shape, speed, topDark, bottomDark, topLight, bottomLight, foamColor, addFoam, opacity) =>
{
	if (id == -1)
		id = RPM.Core.ReactionInterpreter.currentObject.id;
	createWaterfall(id, diameter, height, shape, speed, topDark, bottomDark, topLight, bottomLight, foamColor, addFoam, opacity);
});
