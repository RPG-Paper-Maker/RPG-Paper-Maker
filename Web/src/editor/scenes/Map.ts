...
class Map extends Base {
    ...
    async load() {
        ...
        let file = await LocalFile.getFile(folderMap + Paths.FILE_MAP_INFOS);
        if (file) {
        ...
        this.meshPlane = new THREE.Mesh(new THREE.PlaneBufferGeometry(Project
            .getSquareSize() * this.modelMap.length, Project.getSquareSize() * 
            this.modelMap.width, 1), material);
        ...
        await mapPortion.load(folderMap + this.mapPortion.getFileName());
    ...
    async save() {
        ...
        if (Project.current) {
            ...
            const mapName = Model.Map.generateMapName(this.id);
            const folderMap = Project.current!.getPathMaps() + '/' + mapName;
            await this.modelMap.save(folderMap + Paths.FILE_MAP_INFOS);

            // Save portions
            await this.mapPortion.save(folderMap + this.mapPortion.getFileName());
        }
        this.loading = false;
    }
