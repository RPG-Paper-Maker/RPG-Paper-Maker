...
class Portion {
    ...
    static createFromVector3(position: THREE.Vector3): Portion {
        return new Portion(Math.floor(position.x / Project.getSquareSize() / 
            Constants.PORTION_SIZE), Math.floor(position.y / Project.getSquareSize() / 
            Constants.PORTION_SIZE), Math.floor(position.z / Project.getSquareSize() / 
