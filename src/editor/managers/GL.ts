...
class GL {
	...
	public static textureLoader = new TextureLoader();
	...
	static async loadTexture(path: string): Promise<MeshPhongMaterial> {
		const texture: Texture = await new Promise((resolve) => {
	...
	static loadTextureEmpty(): MeshPhongMaterial {
	...
	static getMaterialTextureSize(material: MeshPhongMaterial | null): { width: number; height: number } {
		return {
	...
	static createMaterial(opts: {
		...
		if (!opts.texture) {
			opts.texture = new Texture();
		}
		opts.texture.magFilter = NearestFilter;
		opts.texture.minFilter = NearestFilter;
		...
		const side = Utils.defaultValue(opts.side, DoubleSide);
		...
		material.userData.customDepthMaterial = new MeshDepthMaterial({
