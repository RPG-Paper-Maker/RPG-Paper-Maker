...
class Sprite extends Base {
    ...
    static addStaticSpriteToGeometry(geometry: CustomGeometry, vecA: THREE.Vector3, 
        vecB: THREE.Vector3, vecC: THREE.Vector3, vecD: THREE.Vector3, texA: THREE
        .Vector2, texB: THREE.Vector2, texC: THREE.Vector2, texD: THREE.Vector2, 
        count: number): number {
    ...
    updateGeometry(geometry: CustomGeometry | CustomGeometryFace, width: number, 
        height: number, position: Position, count: number, tileset: boolean, 
        localPosition: THREE.Vector3 | null): number {
        ...
        let size = new THREE.Vector3(this.texture.width * Project.getSquareSize() * 
            position.scaleX, this.texture.height * Project.getSquareSize() *
            position.scaleY, 1.0);
        ...
        // Double sprite
        if (this.kind === Enum.ElementMapKind.SpritesDouble || this.kind ===
            Enum.ElementMapKind.SpritesQuadra) {
            let vecDoubleA = vecA.clone();
            let vecDoubleB = vecB.clone();
            let vecDoubleC = vecC.clone();
            let vecDoubleD = vecD.clone();
            Base.rotateQuad(vecDoubleA, vecDoubleB, vecDoubleC, vecDoubleD,
            ...
            if (this.kind === Enum.ElementMapKind.SpritesQuadra) {
                ...
                Base.rotateQuad(vecQuadra1A, vecQuadra1B, vecQuadra1C,
                    vecQuadra1D, center, 45, Sprite.Y_AXIS);
                Base.rotateQuad(vecQuadra2A, vecQuadra2B, vecQuadra2C,
