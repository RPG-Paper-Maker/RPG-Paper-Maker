...
class Land extends Base {
    ...
    updateGeometryLand(geometry: CustomGeometry, position: Position, width: number, 
        height: number, x: number, y: number, w: number, h: number, count: number)
    {
        ...
        let vecC = new THREE.Vector3(a + Project.getSquareSize(), b, 
            c + Project.getSquareSize());
        let vecD = new THREE.Vector3(a, b, c + Project.getSquareSize());
        let center = new THREE.Vector3(a + (Project.getSquareSize() 
            / 2), b, c + (Project.getSquareSize() / 2));
