...
function SubMenu({ children, title, icon }: Props) {
    ...
    useEffect(() => {
        if (refMain.current && refArrow.current && refMain.current.parentElement) {
            ...
            if (refTitle.current && refContent.current) {
                ...
                refContent.current.setAttribute('style', 'top:'+ top + 'px; left:' 
                    + left + 'px;');
            }
        }
    });
    ...
    return (
        <div ref={refMain} className='custom-sub-menu'>
            <div ref={refTitle} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} 
                onMouseOver={onMouseOver} onClick={onClickInSub}
            ...
            <div className='absolute' ref={refContent} onClick={onClick}>
                <SubMenuContent subVisible={subVisible} setSubVisible={setSubVisible}
                    testVisible={testVisible} setTestVisible={setTestVisible}
                    setOpened={setOpened}>
                    {children}
                </SubMenuContent>
            </div>
