...
function tokenize(text) {
	...
	for (var i = 0; i < text.length; i++) {
		if (text[i] == '+') expr.push({ token: Token.ADD, type: 'op', value: null });
		else if (text[i] == '*') {
			if (text[++i] == '*') expr.push({ token: Token.POW, type: 'op', value: null });
			else {
				expr.push({ token: Token.MULT, type: 'op', value: null });
				i--;
			}
		} else if (text[i] == '/') expr.push({ token: Token.DIV, type: 'op', value: null });
		else if (text[i] == '%') expr.push({ token: Token.MOD, type: 'op', value: null });
		else if (text[i] == '(') expr.push({ token: Token.OPEN, type: null, value: null });
		else if (text[i] == ')') expr.push({ token: Token.CLOSE, type: null, value: null });
		else if (text[i] == ',') expr.push({ token: Token.COMMA, type: 'op', value: null });
		else if (text[i] == 'l') {
			if (text[++i] == 'n') expr.push({ token: Token.LN, type: 'func', value: null });
			else return null;
		} else if (text[i] == 's') {
			if (text[++i] == 'i') {
				if (text[++i] == 'n') expr.push({ token: Token.SIN, type: 'func', value: null });
				else return null;
			} else if (text[i] == 'q') {
				if (text[++i] == 'r') {
					if (text[++i] == 't') expr.push({ token: Token.SQRT, type: 'func', value: null });
					else return null;
				} else {
			...
			if (text[++i] == 'o') {
				if (text[++i] == 's') expr.push({ token: Token.COS, type: 'func', value: null });
				else return null;
			} else return null;
		} else if (text[i] == 'a') {
			if (text[++i] == 'b') {
				if (text[++i] == 's') expr.push({ token: Token.ABS, type: 'func', value: null });
				else return null;
			} else return null;
		} else if (text[i] == 't') {
			if (text[++i] == 'a') {
				if (text[++i] == 'n') expr.push({ token: Token.TAN, type: 'func', value: null });
				else return null;
			} else {
			...
			if (text[++i] == 'o') {
				if (text[++i] == 'u') {
					if (text[++i] == 'n') {
						if (text[++i] == 'd') expr.push({ token: Token.ROUND, type: 'func', value: null });
						else return null;
					} else return null;
				} else return null;
			} else if (text[i] == '2') {
			...
			if (text[++i] == 'i') {
				if (text[++i] == 'n') expr.push({ token: Token.MIN, type: 'func', value: null });
				else return null;
			} else if (text[i] == 'a') {
				if (text[++i] == 'x') expr.push({ token: Token.MAX, type: 'func', value: null });
				else return null;
			} else return null;
		} else if (text[i] == 'v') {
			if (text[++i] == 'a') {
				if (text[++i] == 'r') expr.push({ token: Token.VAR, type: 'func', value: null });
				else return null;
			} else return null;
		} else {
			var num = '';
			while (num.search(/^-?\d*(\.\d*)?$/) === 0 && i < text.length) num += text[i++].toString();
			if (num.search(/^-?\d*(\.\d*)?$/) !== 0) {
				num = num.slice(0, -1);
				i--;
			}
			i--;
			if (num.length > 0) {
				if (num == '-') expr.push({ token: Token.SUB, type: 'op', value: null });
				else {
	...
	for (var i = 0; i < expr.length; i++) {
		if (expr[i].token === Token.OPEN) {
			if (i < expr.length - 1 && expr[i + 1].token === Token.CLOSE) return null;
			else par++;
		} else if (expr[i].token === Token.CLOSE) par--;
		else if (i < expr.length - 1) {
			if (expr[i].type === expr[i + 1].type) return null;
			if (expr[i].type == 'func' && expr[i + 1].type !== null) return null;
		}
		if (par < 0) return null;
	}
	if (par === 0) return buildTree(expr);
	return null;
}

function buildTree(expr) {
	if (expr[0].type == 'op' || expr[expr.length - 1].type == 'op') return null;
	var tree = [];
	for (var i = 0; i < expr.length; i++) {
		if (expr[i].token === Token.OPEN) {
			var nested = [];
			...
			for (j = 1; par !== 0 && i + j < expr.length; j++) {
				...
				if (expr[i + j].token === Token.OPEN) par++;
				else if (expr[i + j].token === Token.CLOSE) par--;
			}
			if (par !== 0) return null;
			nested.pop();
			...
			tree.push(node);
			i += j - 1;
		} else tree.push(expr[i]);
	}
	for (var i = 0; i < tree.length; i++) {
		if (!tree[i]) return null;
		if (tree[i].type == 'func') tree[i].left = tree.splice(i + 1, 1)[0];
	}
	for (var i = 0; i < tree.length; i++) {
...
function evaluate(expr, t, r) {
	...
	if (Array.isArray(left) && expr.token !== Token.MIN && expr.token !== Token.MAX) return null;
	switch (expr.token) {
		case Token.ADD:
			return left + evaluate(expr.right, t, r);
		case Token.SUB:
			return left - evaluate(expr.right, t, r);
		case Token.MULT:
			return left * evaluate(expr.right, t, r);
		...
		case Token.MOD:
			return left % evaluate(expr.right, t, r);
		case Token.POW:
			return Math.pow(left, evaluate(expr.right, t, r));
		case Token.SQRT:
		...
		case Token.COMMA:
			return [left].concat([evaluate(expr.right, t, r)]);
	}
}
...
setInterval(function () {
	if (Manager.Stack.top instanceof Scene.Map && !Scene.Map.current.loading) {
		...
		for (var i = 0; i < emitterList.length; i++) {
			...
			if (e.map === Scene.Map.current && !e.origin.removed) {
			...
			} else emitterList.splice(i--, 1);
		}
	}
}, 16);

function endParticles(object, smooth) {
	if (object == -1) object = Core.ReactionInterpreter.currentObject.id;
	else if (object == 0) object = Core.Game.current.hero.id;
	for (var i = 0; i < emitterList.length; i++) {
		const e = emitterList[i];
		if (e.id === object) {
			e.rate = 0;
...
Manager.Plugins.registerCommand(
	...
	(object, rate, lifespan, position, rotation, size, opacity, texture, additiveBlending) => {
