...
setInterval(function () {
	if (Manager.Stack.top instanceof Scene.Map && !Scene.Map.current.loading) {
		if (Scene.Map.current !== lastMap) {
			...
			mountList = [];
			obj3dList = [];
			const walls = Scene.Map.current.texturesWalls;
			const mount = Scene.Map.current.texturesMountains;
			const obj3d = Scene.Map.current.texturesObjects3D;
			for (var i = 0; i < walls.length; i++) if (!!walls[i]) wallsList.push(walls[i]);
			...
			for (var i = 0; i < obj3d.length; i++) if (!!obj3d[i]) obj3dList.push(obj3d[i]);
...
Scene.Map.prototype.updateCameraHiding = function (pointer) {
	...
	const dir = (target ? this.camera.target.position : Core.Game.current.hero.position)
		.clone()
		.add(new THREE.Vector3(0, 0.5 * Datas.Systems.SQUARE_SIZE, 0))
			...
			if (Math.ceil(intersects[i].distance) + 5 < this.camera.distance) {
				...
				if (
					...
					intersects[i].object.material.userData.uniforms.viewBlockPlugin_squareSize = {
						value: Datas.Systems.SQUARE_SIZE,
					};
