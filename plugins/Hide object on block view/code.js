...
setInterval(function ()
{
	if (Manager.Stack.top instanceof Scene.Map && !Scene.Map.current.loading)
	{
		...
		for (var i = 0; i <= Scene.Map.current.maxObjectsID; i++)
		{
			...
			if (!!obj)
			{
				if (obj.x >= Scene.Map.current.mapProperties.length)
					continue;
				if (obj.y >= Scene.Map.current.mapProperties.height)
					continue;
				if (obj.z >= Scene.Map.current.mapProperties.width)
					continue;
				...
				}, null);//Core.ReactionInterpreter.currentObject);
			}
		}
	}
}, 1000);
...
Scene.Map.prototype.updateCameraHiding = function (pointer)
{
	...
	const dir = (target ? this.camera.target.position : Core.Game.current.hero.position).clone().add(new THREE.Vector3(0, 0.5 * Datas.Systems.SQUARE_SIZE, 0)).sub(this.camera.getThreeCamera().position);
	while (dir.y < 0)
	{
		dir.y += 0.01;
		...
		{
			if (intersects[i].object === Core.Game.current.hero.mesh)
				continue;
			...
			if (Math.ceil(intersects[i].distance) + 5 < this.camera.distance)
			{
				...
				if ((v[eraseMount] && m) || (v[eraseWalls] && w) || (v[eraseObj3D] && o) || (e && intersects[i].object.viewBlockPlugin_erase) || (v[eraseFloor] && !w && !m && !e && !o))
				{
					...
					intersects[i].object.material.userData.uniforms.viewBlockPlugin_squareSize = {value: Datas.Systems.SQUARE_SIZE};
					intersects[i].object.material.userData.uniforms.viewBlockPlugin_tgtPos = {value: (target ? this.camera.target.position : Core.Game.current.hero.position).clone().add(this.camera.targetOffset).add(customOffset)};
				}
			}
		}
	}
...
{
	if (!!Manager.GL.SHADER_FIX_VERTEX && !!Manager.GL.SHADER_FIX_FRAGMENT && !!Manager.GL.SHADER_FACE_VERTEX && !!Manager.GL.SHADER_FACE_FRAGMENT)
	{
		...
		{
			Common.Platform.showErrorMessage("There is an error in your shader files. Fix it before you can use this plugin.");
