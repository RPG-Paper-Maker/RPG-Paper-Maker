...
inject(
	...
	'load',
	async function () {
		const flippableAnimations = Manager.Plugins.getParameter(pluginName, 'FlippableAnimations');
		for (const anim of flippableAnimations) {
			for (const frame of anim.frames) {
				...
				for (const element of frame.elements) {
					...
					if (!element.isPartyTarget)
						element.isPartyTarget = function () {
					...
					const flipGetter =
						Object.getOwnPropertyDescriptor(element, 'flip').get ||
						function () {
							return this._jollyFlip;
						};
					const flipSetter =
						Object.getOwnPropertyDescriptor(element, 'flip').set ||
						function (value) {
					...
					const xGetter =
						Object.getOwnPropertyDescriptor(element, 'x').get ||
						function () {
							return this._jollyX;
						};
					const xSetter =
						Object.getOwnPropertyDescriptor(element, 'x').set ||
						function (value) {
							this._jollyX = value;
						};
